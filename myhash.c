...
void hash(char * textregular, char * buffer){ // buffer is String (printable) form of the raw hash - printf((const char*)rawHash); is nonsense
	...
	SHA256_CTX generator;
	sha256_init(&generator);
	...
	sha256_update(&generator, text, strlen((const char*)text)); // prepare generator
	sha256_final(&generator, rawHash); // actual hash algoritem and put hash in rawhash
